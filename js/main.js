...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const addHighlightTool = () => {
    ...
    `
        ...
        toastr.success("如果转载，请保留原文链接哦","复制成功！！！");  
      }
    }
    ...
    const copy = async (text, ctx) => {
      try {
        ...
        toastr.error(GLOBAL_CONFIG.copy.noSupport)
      }
    }
 ...
 */
  ...
  const justifiedIndexPostUI = () => {
    ...
    const init = () => {
      masonryItemGlobal = new InfiniteGrid.MasonryInfiniteGrid('.recent-post-items', {
        gap: { horizontal: 10, vertical: 20 },
        useTransform: true,
        useResizeObserver: true
      })
      masonryItemGlobal.renderItems()
      btf.addGlobalFn('pjaxCompleteOnce', () => {
        if (masonryItemGlobal) {
          masonryItemGlobal.destroy()
          masonryItemGlobal = null
        }
...
/**
  ...
  const setCategoryBarActive = () => {
    ...
    const postsContainer = document.querySelector('.recent-post-items');
    if (!postsContainer) return;

    let currentFilterCategory = 'all';

    const filterPosts = (category) => {
      const allPostItems = postsContainer.querySelectorAll(".recent-post-item:not(.ads-wrap)");
      allPostItems.forEach(item => {
        const itemCats = item.getAttribute("data-categories") || '';
        if (category === "all") {
          item.style.display = "";
        } else {
          const catList = itemCats.split(",").map(c => c.trim()).filter(Boolean);
          item.style.display = catList.includes(category) ? "" : "none";
        }
    ...
    const createMasonry = () => {
      const visibleItems = postsContainer.querySelectorAll('.recent-post-item:not(.ads-wrap)');
      ...
      if (!hasVisible) {
        postsContainer.removeAttribute('style');
        return;
      }

      if (typeof InfiniteGrid === 'undefined') {
        btf.getScript(GLOBAL_CONFIG.infinitegrid.js).then(() => {
          masonryItemGlobal = new InfiniteGrid.MasonryInfiniteGrid('.recent-post-items', {
            gap: { horizontal: 10, vertical: 20 },
            useTransform: true,
            useResizeObserver: true
          ...
          masonryItemGlobal.renderItems();
          // InfiniteGrid 重新渲染后会覆盖 display:none，需要重新应用过滤
          filterPosts(currentFilterCategory);
        }).catch(() => {});
        return;
      }
    ...
    const handleCategoryClick = (e) => {
      const link = e.target.closest(".category-bar-link");
      if (!link) return;

      const category = link.getAttribute("data-category");
      if (category === null) return; // "文章存档"没有 data-category，保持原链接跳转

      e.preventDefault();
      e.stopPropagation();

      categoryItems.forEach(item => item.classList.remove("select"));
      const parentItem = link.closest(".category-bar-item");
      if (parentItem) parentItem.classList.add("select");
